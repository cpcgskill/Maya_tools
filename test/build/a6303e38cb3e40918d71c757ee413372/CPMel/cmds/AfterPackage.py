...
def reconstructionCurve(curve, size=10, d=1):
    ...
    max_u = curve.numSpans()
    max_size = (size - 1)
    cmds.curve(str(curve), r=True, p=[tuple(curve.getPointAtParam(((float(i) / max_size) * max_u))) for i in range(size)], d=d)
...
def attachToModel(mesh=node.Mesh, curve=node.Curve):
    ...
    if (not isinstance(curve, node.DgNode)):
        curve = node.newObject(curve)
    [curve.setPoint(i, mesh.getClosestPointAndFace(curve.getPoint(i, node.Space.world), node.Space.world)[0], node.Space.world) for i in range(curve.numCvs())]
